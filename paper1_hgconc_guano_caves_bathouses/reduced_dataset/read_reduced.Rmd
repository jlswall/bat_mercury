...
First, read into R the data from the "guano_caves" and "guano_bat_houses" sheets from the Excel workbook (Hg_DATA_ActaChirp.xlsx).
```{r}
library("openxlsx")
...
cavesDF <- read.xlsx(xlsxFile = fileWpath, sheet="guano_caves", detectDates=TRUE)
## Rename the columns to shorter names.
colnames(cavesDF) <- c("Cave", "Species", "Date", "Region", "Notes", "OM", "Mercury")
## The second sheet contains measurements from bat houses.
housesDF <- read.xlsx(xlsxFile = fileWpath, sheet="guano_bat houses", detectDates=TRUE)
colnames(housesDF) <- c("BatHouse", "Species", "Date", "Region", "Notes", "OM", "Mercury")
```
...
indicCore <- grepl("core", cavesDF$Notes, ignore.case=TRUE)
## Initialize coreID and coreOrder variables:
cavesDF$coreID <- NA
cavesDF$coreOrder <- NA


## For Climax Cave's core:
cavesDF$coreID[11:20] <- "ClimaxCore"
cavesDF$coreOrder[11:20] <- 1:10

## For Cottondale's core:
cavesDF$coreID[27:32] <- "CottondaleCore"
cavesDF$coreOrder[27:32] <- 1:6

## For Florida Caverns Old Indian Cave, there appear to be 2 8-inchcores:
cavesDF$coreID[33:40] <- "OldIndianCore1"
cavesDF$coreOrder[33:40] <- 1:8
cavesDF$coreID[41:48] <- "OldIndianCore2"
cavesDF$coreOrder[41:48] <- 1:8

## For Judge's Cave, there are 1 11-inch core and 1 8-inch core:
cavesDF$coreID[66:76] <- "JudgesCore1"
cavesDF$coreOrder[66:76] <- 1:11
cavesDF$coreID[77:84] <- "JudgesCore2"
cavesDF$coreOrder[77:84] <- 1:8
