...
with(excl4DF,
     plot(c(1.0, 5.5), c(0, max(MercuryConc)), type="n", xaxt="n", yaxt="n",
          xlab="Region", ylab="Mercury concentration (ppm)",
          cex.lab=0.8)
     )
## Make boxplot for all regions combined.
boxplot(excl4DF[,"MercuryConc"], at=1.5, add=TRUE, col="gray", axes=F) ## width=0.8)
text(1.5, 1.05, paste0("n=", nrow(excl4DF)), cex=0.7, col=gray(0.4))
## Break down by region, plot each region separately.
for (i in 1:3){
  subDF <- subset(excl4DF, as.numeric(Region)==i)
...
axis(1, at=c(1.5, 2+c(1:3)), labels=c("Combined", c(1:3)),
...
## ###########################################################
## TASK 1: Find the overall mean, excluding region 4.
...
## All data except region 4 data:
t.test(excl4DF[,"MercuryConc"])  ## 95% CI: (0.4948967, 0.5871363)

## Excluding outliers and region 4 data:
t.test(noExtrDF[,"MercuryConc"])  ## 95% CI: (0.4807534, 0.5438820)
## ##########################
...
## Using data from caves in regions 1-3, outliers intact:
t.test(subset(excl4DF, loctype=="cave", "MercuryConc"))
## 95% CI: (0.5099824, 0.6121885)

## As above, but excluding outliers.
t.test(subset(excl4DF, (loctype=="cave") & (MercuryConc < 1.2), "MercuryConc"))
## 95% CI: (0.4948638, 0.5609612)
## ##########################
## ###########################################################
